pw.component.register('content-editor-embed', function (view, config) {
  ...
  this.transform = function (value) {
    if (value.search('player.vimeo.com/video/') != -1) {
      ...
      pw.node.after(view.node, div);
      view.node.classList.add('hidden');
    } else if (value.search('youtube.com/watch') != -1) {
      ...
      pw.node.after(view.node, div);
      view.node.classList.add('hidden');
    } else if (value.search('open.spotify.com/track') != -1) {
      var id = value.split('open.spotify.com/track/')[1];
      var source = '<iframe src="//embed.spotify.com/?uri=spotify%3Atrack%3A' + id + '" frameborder="0" allowtransparency="true"></iframe>';
      var div = document.createElement('div');
      div.classList.add('console-content-spotify-wrapper');
      div.innerHTML = source;
      pw.node.after(view.node, div);
      view.node.classList.add('hidden');
    }
