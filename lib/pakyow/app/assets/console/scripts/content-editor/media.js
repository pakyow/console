pw.component.register('content-editor-media', function (view, config, name, id) {
  ...
  var blinder, media, closebtn;

  var input = view.node.querySelector('input');
  var icon = view.node.querySelector('i').cloneNode(true);
  // var count = view.node.querySelector('.console-content-image-count').cloneNode(true);
  ...
  this.listen(channel + ':navigator:enter', function (response) {
    if (!blinder) {
      ...
      closebtn.classList.add('ui-modal-close');
      closebtn.innerHTML = '<i class="fa fa-times-circle" data-ui="modal-close"></i>';
    ...
    media.insertAdjacentHTML('beforeend', response.body);
  ...
  this.transform = function (items) {
    ...
    if (constraints) {
      ...
      constrainedHeight = constraints.height / resizedScaleFactor;
    } else {
    ...
    if (items.length == 0) {
      ...
      // pw.node.remove(view.node.querySelector('.console-content-image-count'));
    } else {
      if (i) {
        pw.node.remove(i);
      }
      ...
      // var icount = count.cloneNode(true);
      // icount.innerHTML = items.length;
      // pw.node.append(view.node, icount);
    }
