  ...
  this.transform = function (state) {
    ...
    if (state.length > 0) {
      ...
      state.forEach(function (datum) {
        ...
        componentViewsToInit.push($match);
      });

      this.view.scope('content').bind(state);

      componentViewsToInit.forEach(function (view) {
    ...
    state.forEach(function (datum) {
      ...
      if (constraints && constraints[datum.type] && constraints[datum.type][datum.align] && constraints[datum.type][datum.align][datum.step]) {
        $editorView.constraints = constraints[datum.type][datum.align][datum.step];
      }
    });
  };
});
