pw.component.register('content-editor-media', function (view, config, name, id) {
  ...
  this.load = function (push) {
    ...
    self.listen('content-editor:align:toggle', function () {
      self.transform();
    });

    self.listen('content-editor:stepper:stepped', function () {
      self.transform();
    });
  };
  ...
  var cachedTransformItems;
  this.transform = function (transformItems) {
    if (transformItems) {
      cachedTransformItems = transformItems;
    }

    if (!cachedTransformItems) {
      return;
    }

    var i = view.node.querySelector('i');

    if (cachedTransformItems.length == 0) {
      if (!i) {
        pw.node.append(view.node, icon.cloneNode(true));
      }

      view.node.style.backgroundImage = null;
    } else if (cachedTransformItems.length == 1) {
      if (i) {
        pw.node.remove(i);
      }

      var firstItem = cachedTransformItems[0];
    ...
    view.node.querySelector('input').value = JSON.stringify(cachedTransformItems);
